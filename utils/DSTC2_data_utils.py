...
def prepare_dataset(data_path, tokenizer, slot_meta, n_history, max_seq_length, data_type=''):
    ...
    for dial_dict in dials:
        ...
        for ti, turn in enumerate(dial_dict["turns"]):
            ...
            if data_type == 'train':
                user_uttr = turn['user_transcript'].lower().strip()
            else:
                user_uttr = turn['user_asr'].lower().strip()
            user_uttr = process_uttr(user_uttr)

            base_bio_usr = make_bio_and_value_position(turn['turn_labels'], user_uttr)
            system_uttr = turn['system_transcript'].lower().strip()
            system_uttr = process_uttr(system_uttr)
            base_bio_sys = make_bio_and_value_position(turn['turn_labels'], system_uttr)
            if system_uttr == '':
                turn_uttr = user_uttr
